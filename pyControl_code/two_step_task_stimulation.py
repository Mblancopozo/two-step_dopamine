...
# Stimulation parametres
v.stimulation_type = 'baseline' # (no stim) or 'ss_cue' or 'outcome_cue'
v.power_mW = 7
v.n_pulses = 5
v.freq_stim = 25
v.duty_cycle = 25
v.fraction_stim_trials = 0.25 # Fraction of all trials that are stimmed.
v.min_non_stim_after_stim = 2 # Minimum number of non-stim trials after a stim trial.

# Stimulation variables.
# v.stim_prob_when_allowed = 1/((1/v.fraction_stim_trials) - v.min_non_stim_after_stim)
v.trials_till_stim_allowed = 0
v.current_stim = False
v.n_stim = 0
...
def cue_up_state(event):
    if event == 'entry':
        if v.current_stim and v.stimulation_type == 'ss_cue':
            hw.stim.pulse(freq=v.freq_stim, duty_cycle=v.duty_cycle, n_pulses=v.n_pulses)
            print('Stimulation ON - ss_cue - freq:{}, dcycle:{}, npulses:{}, power:{}'.format(
                v.freq_stim, v.duty_cycle, v.n_pulses, v.power_mW))
            v.n_stim += 1
        hw.poke_1.LED.on()
...
def cue_down_state(event):
    if event == 'entry':
        ...
        if v.current_stim and v.stimulation_type == 'ss_cue':
            hw.stim.pulse(freq=v.freq_stim, duty_cycle=v.duty_cycle, n_pulses=v.n_pulses)
            print('Stimulation ON - ss_cue - freq:{}, dcycle:{}, npulses:{}, power:{}'.format(v.freq_stim, v.duty_cycle,
                                                                                              v.n_pulses, v.power_mW))
            v.n_stim += 1
...
def reward_cue(event):
    if event == 'entry':
        ...
        if v.current_stim and v.stimulation_type == 'outcome_cue':
            hw.stim.pulse(freq=v.freq_stim, duty_cycle=v.duty_cycle, n_pulses=v.n_pulses)
            print('Stimulation ON - outcome_cue - freq:{}, dcycle:{}, npulses:{}, power:{}'.format(
                v.freq_stim, v.duty_cycle, v.n_pulses, v.power_mW))
            v.n_stim += 1
        if v.second_step == 'up':
...
def trial_update():
    ...
    if v.second_step is 'up':
        ...
        second_print = 0
    print('T#:{} R#:{} B#:{} C:{} S:{} O:{} CA:{} B:{} CT:{} TS:{} stim:{} stype:{} nstim:{}'.format(v.n_trials,
    v.n_rewards, v.n_blocks, choice_print, second_print, v.outcome,
    v.print_CA, v.current_block, v.current_choice_type, v.stage, int(v.current_stim), v.stimulation_type, v.n_stim))
    ...
    # Decide next trial type.
    if v.current_stim: # Next trial needs to be free-choice
        v.current_choice_type = 'FC'
    else:
        v.current_choice_type = v.choice_type.next()
    # Decide if next trial is stimmed.
    if v.current_stim:
        v.trials_till_stim_allowed = v.min_non_stim_after_stim
        v.current_stim = False
    elif v.trials_till_stim_allowed > 0:
        v.trials_till_stim_allowed -= 1
        v.current_stim = False
    else:
        if v.stimulation_type != 'baseline':
            v.current_stim = withprob(v.fraction_stim_trials)
        else:
            v.current_stim = False
...
def block_transition():
    if v.start_block == False:
        ...
        # if not next_block: #if there is no block available, add annother set of blocks.
