...
@njit
def latent_state_session_value(choices, second_steps, outcomes, n_trials, transition_type, params):
  # Unpack parameters.
  ...
  p_o_1 = np.array([[good_prob, 1 - good_prob],  # Probability of observed outcome given world in state 1.
                    [1 - good_prob, good_prob]])  # Indices:  p_o_1[second_step, outcome]
...
def latent_state_session_simulate(n_trials, task, param_names, params, kernels):
  # Unpack parameters.
  p_r, weight_inf = params[:2]
...
class Latent_state(RL_agent):
  ...
  def session_simulate(self, task, params, n_trials):
    return latent_state_session_simulate(n_trials, task, self.param_names, params, self.use_kernels)
